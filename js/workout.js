const Workout = {
  ...
  renderDay(dayId) {
    ...
    day.exercises.forEach((ex, exIdx) => {
      ...
      html += `<div class="card" id="ex-${exIdx}">
        <div class="exercise-header">
          <div style="flex:1">
            ...
            <div class="exercise-target">${ex.sets}x${ex.repsTarget} | Descanso: ${ex.rest}s</div>
            <div class="exercise-material">${ex.material}</div>
          </div>
          <div class="timer-inline">
            <span class="timer-bar-display hidden" id="timer-display-${exIdx}">0:00</span>
            <button class="timer-exercise-btn" data-ex="${exIdx}" data-rest="${ex.rest}" title="Descanso ${ex.rest}s">
              ${this._svgTimerOn}
            </button>
          </div>
        </div>`;
      ...
      if (ex.notes) {
        html += `<div style="font-size:0.75rem;color:var(--text-dim);margin-bottom:10px;font-style:italic">${ex.notes}</div>`;
      }

      html += `<div class="set-grid-v2">
        <div class="set-grid-header-row">
          <div class="sg-h"></div>
          <div class="sg-h">Reps</div>
      ...
      for (let s = 0; s < ex.sets; s++) {
        ...
        html += `<div class="set-row-v2">
          ...
          <input type="number" class="set-input set-input-small" data-ex="${exIdx}" data-set="${s}" data-field="reps" placeholder="${placeholder_r}" inputmode="numeric" min="0">
          <input type="number" class="set-input set-input-small" data-ex="${exIdx}" data-set="${s}" data-field="weight" placeholder="${placeholder_w}" inputmode="decimal" step="0.5" min="0">
  ...
  _bindEvents(area, dayId) {
    ...
    document.getElementById('sync-workout')?.addEventListener('click', () => {
      ...
      a.download = `sync-${user?.name || 'data'}-${new Date().toISOString().slice(0,10)}.json`;
      a.click();
  ...
  _autosave(dayId) {
    ...
    this._autosaveTimeout = setTimeout(() => {
      ...
      const hasData = data.exercises.some(ex => ex.sets.some(s => s.reps !== null || s.skipped));
      if (!hasData) return;
  ...
  _collectWorkoutData(dayId) {
    ...
    day.exercises.forEach((ex, exIdx) => {
      ...
      for (let s = 0; s < ex.sets; s++) {
        ...
        sets.push({
          reps: repsInput?.value ? parseFloat(repsInput.value) : null,
