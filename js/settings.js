  ...
  render() {
    ...
    let html = `
      <div class="settings-section">
        ...
        `).join('')}
        <button class="btn btn-secondary btn-small" id="save-users" style="margin-top:8px">Guardar nombres</button>
      </div>

      <div class="settings-section">
        <h3>PIN de acceso</h3>
        <p style="font-size:0.85rem;color:var(--text-dim);margin-bottom:8px">
          ${pin ? 'PIN configurado. Cambiar:' : 'Sin PIN. Cualquiera puede acceder.'}
        </p>
        <div style="display:flex;gap:8px;align-items:center">
          <input type="password" class="set-input" style="width:120px" id="pin-input" placeholder="4 digitos" maxlength="4" inputmode="numeric">
          <button class="btn btn-secondary btn-small" id="save-pin">Guardar PIN</button>
          ${pin ? '<button class="btn btn-small btn-danger" id="remove-pin">Quitar</button>' : ''}
        </div>
      </div>
  ...
  _bindEvents() {
    document.getElementById('save-users')?.addEventListener('click', () => {
      ...
      UI.showModal(`
        <div style="text-align:center">
          <h3 style="color:var(--success);margin-bottom:12px">Nombres guardados</h3>
    ...
    document.getElementById('save-pin')?.addEventListener('click', () => {
      ...
      if (pin.length !== 4 || !/^\d{4}$/.test(pin)) {
        UI.showModal(`
          <h3 style="margin-bottom:12px">PIN invalido</h3>
          <p style="color:var(--text-dim);margin-bottom:16px">El PIN debe ser de 4 digitos numericos.</p>
      ...
      UI.showModal(`
        <div style="text-align:center">
          <h3 style="color:var(--success);margin-bottom:12px">PIN guardado</h3>
          <button class="btn btn-primary" onclick="UI.hideModal()">OK</button>
        </div>
      `);
      Settings.render();
    ...
    document.getElementById('clear-data')?.addEventListener('click', () => {
      UI.showModal(`
        <h3 style="color:var(--danger);margin-bottom:12px">Borrar todos los datos?</h3>
        <p style="color:var(--text-dim);margin-bottom:16px">Se eliminaran todos los entrenos del usuario actual. Esta accion no se puede deshacer.</p>
        <div class="btn-group">
